/* Generate social images off the OM cover look (outlined aerial + title + stats).
   Outputs: South-End-Plaza-social-linkedin.png (1200x1200) and
            South-End-Plaza-social-instagram.png (1080x1350). */
...
const html = (W, H, layout = 'top') => {
  const u = W / 1080;                 // scale unit
  const pad = Math.round(64 * u);
  const block = `
        <div class="eyebrow">17-Unit Mixed-Use &middot; Value-Add</div>
        <div class="title">South End<br>Plaza</div>
        <div class="rule"></div>`;
  const body = layout === 'bottom'
    ? `
      <img class="logo" src="${LOGO}">
      <div class="foot">${block}
        <div class="addr" style="margin-top:${Math.round(20*u)}px">310 South Main Street &middot; Thomaston, CT 06787</div>
      </div>`
    : `
      <img class="logo" src="${LOGO}">
      <div class="head">${block}</div>
      <div class="foot">
        <div class="addr">310 South Main Street &middot; Thomaston, CT 06787</div>
      </div>`;
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box;-webkit-font-smoothing:antialiased}
    ...
    <div class="card">
      <div class="bg"></div><div class="scrim"></div>${body}
    </div>
  </body></html>`;
};
...
// 'top' keeps the original title-up-top layout; 'bottom' drops the title block to
// the foot. Bottom files get a -bottom suffix so both versions are kept.
...
(async () => {
  ...
  for (const layout of layouts) {
    for (const s of sizes) {
      ...
      await page.setContent(html(s.w, s.h, layout), { waitUntil: 'networkidle0' });
      await page.evaluate(() => document.fonts.ready);
      const suffix = layout === 'bottom' ? '-bottom' : '';
      const out = path.join(ROOT, `South-End-Plaza-social-${s.name}${suffix}.png`);
