...
/** Unit-mix summary roll (monthly basis) — Merrimack layout. 53 units
    won't fit line-by-line on one page, so this page mirrors the
    workbook's Unit Mix block (averages per type per column); the full
    per-unit roll follows on the Rent Roll Detail page. */
function UnitRoll({ pageNum }: { pageNum?: number }) {
  ...
  const rentBars = [
    ...
    { label: 'Pro Forma', value: computed.byColumn['proForma']?.annual ?? 0, color: '#F8971D' },
  ]
  const upside =
    rentBars[1]!.value > rentBars[0]!.value
  ...
  return (
    <div className="page">
      ...
      <div className="section--tight" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        <SectionTitle text="Unit Mix &" accent="Rent Summary" />
        ...
        <table className="data-table" style={{ fontSize: 10.5 }}>
          ...
          <tbody>
            {computed.unitMix.map((m) => (
              <tr key={m.use}>
                <td>
                  <strong>{m.use}</strong>
                </td>
                <td className="num">{m.count}</td>
                <td className="num">{m.avgSf != null ? fmtInt(m.avgSf) : '—'}</td>
                {cols.map((c) => (
                  <td className="num" key={c.key}>
                    {m.avgRent?.[c.key] != null ? fmtMoney(m.avgRent[c.key]!) : '—'}
                  </td>
                ))}
              </tr>
            ))}
            <tr className="total-row">
              ...
              {cols.map((c) => (
                <td className="num" key={c.key}>
                  <strong>{fmtMoney(computed.byColumn[c.key]?.annual ?? 0)}</strong>
                </td>
              ))}
            </tr>
            <tr>
              <td colSpan={3}>Average Rent / Unit / Month</td>
              {cols.map((c) => (
                <td className="num" key={c.key}>
                  {computed.avgRentByColumn?.[c.key] != null ? fmtMoney(computed.avgRentByColumn[c.key]!) : '—'}
                </td>
              ))}
            </tr>
          </tbody>
