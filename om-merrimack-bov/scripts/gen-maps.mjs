...
/* ═══════════ MAP GENERATOR (deterministic, no AI) ═══════════
   ...
   Usage: node scripts/gen-maps.mjs [--skip-amenities] [--skip-labels]
          [--coastal]  force raw-polygon band geometry — for inland
          subjects whose 60-min reach hits the coastline (the radial
          envelope would balloon the band over the water)               */
...
if (byTime) {
  ...
  if (args.includes('--coastal') && !coastal) {
    coastal = true
    console.log('  --coastal override → raw-polygon geometry')
  }
