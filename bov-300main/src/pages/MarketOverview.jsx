    ...
    <div className="page">
      ...
      <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
        <div style={{ flex: '0 0 55%', padding: '24px 32px 18px 40px', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--carbon)', marginBottom: 6 }}>{o.subtitle}</div>
          <div className="title-rule" />
          <div style={{ fontSize: 9.3, lineHeight: 1.42, color: 'var(--graphite)', marginBottom: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
            {o.paragraphs.map((p, i) => <p key={i}>{p}</p>)}
          </div>

          <div className="eyebrow" style={{ marginBottom: 5 }}>Demographics — {o.town} vs. {o.state}</div>
          <table className="data-table" style={{ fontSize: 9.4, marginBottom: 5 }}>
            <thead>
              <tr>
                <th>Metric</th>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flex: 1, justifyContent: 'space-between', minHeight: 0 }}>
            {o.employers.map(g => (
              <div key={g.label} style={{ borderLeft: '3px solid var(--golden)', paddingLeft: 12 }}>
                <div style={{ fontSize: 9.3, fontWeight: 800, color: 'var(--carbon)', marginBottom: 1 }}>{g.label}</div>
