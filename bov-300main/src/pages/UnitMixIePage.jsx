...
import { VACANCY_PCT } from '../data/financials.js'
import { FIN, RR, fmtMoney, fmtNum } from '../lib/calc.js'
...
export default function UnitMixIePage({ pageNum }) {
  ...
  return (
    <div className="page">
      <PageHeader section="Unit Mix & I&E" />
      <div className="section--tight" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div className="section-title" style={{ marginBottom: 2, fontSize: 22 }}>Unit Mix &amp; <span style={{ color: '#F8971D' }}>Income Analysis</span></div>
        <div className="title-rule" style={{ marginBottom: 6 }} />

        {/* Donuts */}
        <div style={{ display: 'flex', gap: 14, marginBottom: 8, alignItems: 'stretch' }}>
          <ChartCard title="Unit Mix" data={unitMix} centerLabel={`${RR.unitCount}`} centerSub="UNITS" />
          ...
          <div style={{ padding: '2px 6px', flex: 0.8, display: 'flex', flexDirection: 'column' }}>
            <h3 style={{ fontSize: 10, fontWeight: 700, color: 'var(--carbon)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8, paddingBottom: 6, borderBottom: '2px solid var(--golden)' }}>Avg SF / Type</h3>
        ...
        {/* Income stack */}
        <div className="eyebrow" style={{ marginBottom: 3, fontSize: 9 }}>Underwritten Income &amp; NOI</div>
