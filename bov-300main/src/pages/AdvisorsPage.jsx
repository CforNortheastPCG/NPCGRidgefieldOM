...
export default function AdvisorsPage({ pageNum }) {
  ...
  return (
    <StaticShell section="Your Advisors" title="Your Advisors" pageNum={pageNum}>
      <p style={{ fontSize: 11, lineHeight: 1.55, color: 'var(--carbon)', margin: '0 0 16px', maxWidth: 760 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${n}, 1fr)`, gap: 26, flex: 1, minHeight: 0 }}>
        {ADVISORS.map(a => (
          <div key={a.name} style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
              ...
              {a.photo ? (
                ...
                <div
                  ...
                  style={{
                    ...
                    background: 'var(--carbon)', color: '#fff', fontWeight: 700, fontSize: 20,
                  }}
                >{initials(a.name)}</div>
              )}
              <div style={{ minWidth: 0 }}>
                <div className="dc-name" style={{ fontSize: 16 }}>{a.name}</div>
            ...
            {/* stat tiles */}
            {a.stats?.length > 0 && (
              <div style={{ display: 'grid', gridTemplateColumns: `repeat(${a.stats.length}, 1fr)`, gap: 8, marginBottom: 12 }}>
                {a.stats.map(s => (
                  <div className="stat-tile" key={s.l} style={{ padding: '9px 4px' }}>
                    <div className="st-val" style={{ fontSize: 19 }}>{s.v}</div>
            ...
            {/* track-record bio */}
            {a.bio && (
              <div style={{ fontSize: 10.5, lineHeight: 1.55, color: 'var(--carbon)' }}>
                <Md text={a.bio} />
              </div>
            )}
          </div>
