...
import { CLOSING } from '../data/process.js'
  ...
  return (
    <div className="page">
      ...
      <div className="section--tight" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        ...
        <div style={{ fontSize: 10.3, lineHeight: 1.5, color: 'var(--graphite)', marginBottom: 12 }}>{CLOSING.intro}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: 'repeat(3, 1fr)', gap: 12, flex: 1, minHeight: 0 }}>
          {CLOSING.items.map(it => (
            <div key={it.title} style={{ borderLeft: '3px solid var(--golden)', paddingLeft: 12, display: 'flex', flexDirection: 'column', justifyContent: 'center', minHeight: 0 }}>
              <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--carbon)', marginBottom: 2 }}>{it.title}</div>
              <div style={{ fontSize: 8.8, lineHeight: 1.42, color: 'var(--graphite)' }}>{it.body}</div>
            </div>
          ))}
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginTop: 12, paddingTop: 10, borderTop: '1px solid var(--border)' }}>
          {CLOSING.faq.map(f => (
            <div key={f.q}>
