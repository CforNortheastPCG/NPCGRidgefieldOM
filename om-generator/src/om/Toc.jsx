...
/* ═══════════════════ TABLE OF CONTENTS ═══════════════════
   Page numbers here mirror OmDeck's page order (auto-numbered). If you
   add/remove/reorder pages in OmDeck, update the `n` values below to match. */
...
const TOC = [
  ...
  { n: 7, label: 'Financial Analysis', section: true },
  { n: 8, label: 'Unit Mix & Rent Roll', sub: true },
  { n: 9, label: 'Income & Expense', sub: true },
  { n: 10, label: 'Location & Market', section: true },
  { n: 11, label: 'City Overview', sub: true },
...
export default function Toc({ pageNum }) {
  const deal = useDeal()
  const photo = deal.cover || deal.map
  return (
    <div className="page">
      ...
      <div className="section--tight" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 28, flex: 1, minHeight: 0 }}>
          ...
          {/* Accent photo */}
          <div style={{ borderRadius: 4, overflow: 'hidden', position: 'relative', background: 'var(--carbon)' }}>
