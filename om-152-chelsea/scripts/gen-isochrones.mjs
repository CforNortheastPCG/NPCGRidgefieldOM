/* Drive-time band generator for 152–154 Chelsea St, East Boston — COASTAL raw-polygon method.

   The property sits on Boston Harbor, so a point-centered radial
...
const SUBJECT = { lat: 42.37366, lon: -71.03469 } // 152–154 Chelsea St, East Boston MA
const CONTOURS = [
...
const res = await fetch(`${VALHALLA}?json=${encodeURIComponent(JSON.stringify(body))}`, { headers: { 'User-Agent': 'npcg-om-152-chelsea/1.0' } })
...
const file = `/* AUTO-GENERATED by scripts/gen-isochrones.mjs (coastal raw-polygon method) —
   do not hand-edit. Drive-time bands from 152–154 Chelsea St, East Boston: raw
   Valhalla road-following isochrone polygons (costing auto, so they exclude
   Boston Harbor / Mass Bay), decimated + Chaikin-smoothed, as [lat,lng] arrays.
