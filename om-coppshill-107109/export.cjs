...
(async () => {
  ...
  try {
    ...
    // Post-process: swap Skia's ICC v4 colorspaces to /Device* (Acrobat/Preview
    // render them with a pink cast otherwise) + embed broker metadata.
    await require('./fix-pdf.cjs').fixPdf(out);
  } catch (err) {
    console.error(err);
    process.exitCode = 1;
  } finally {
