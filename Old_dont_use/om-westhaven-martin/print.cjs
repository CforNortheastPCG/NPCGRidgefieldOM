...
const OUT = process.argv[3] || path.join(__dirname, '254-Main-Street-Martin-West-Haven-OM.pdf');
...
const META = {
  title: '254 Main Street, West Haven, CT — 19-Unit Multifamily Offering Memorandum',
  author: 'Northeast Private Client Group',
  subject: 'For Sale · 254 Main Street & 106–108 Martin Street, West Haven, CT 06516 — 19-unit value-add multifamily offering',
  keywords: ['offering memorandum', 'multifamily', 'investment property', 'value-add', 'West Haven CT', 'New Haven County', '254 Main Street', 'Northeast Private Client Group'],
...
(async () => {
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    await pages[i].screenshot({ path: f, type: 'jpeg', quality: 78 });
    files.push(f);
