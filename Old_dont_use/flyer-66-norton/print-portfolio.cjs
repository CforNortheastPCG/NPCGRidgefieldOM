/* Self-contained 66 Norton (Norton Pointe Apartments) closing-flyer exporter — no dev server.
   Reads src/portfolio.js + src/portfolio.css, inlines the brand system and
   local assets (file://), screenshots the page at 2×, composes a one-page
   landscape 11×8.5 PDF + PNG, then embeds AI-facing metadata.
   Usage: NODE_PATH=<...>/node_modules node print-portfolio.cjs */
const fs = require('fs');
const os = require('os');
...
const TMP = fs.mkdtempSync(path.join(os.tmpdir(), 'portfolio-'));
...
async function embedMetadata(out, d) {
  try {
    ...
    const { buildMeta } = require('./portfolio-meta.cjs');
...
(async () => {
  const { PORTFOLIO: d } = await import('./src/portfolio.js');
  const portfolioCss = fs.readFileSync(path.join(ROOT, 'src', 'portfolio.css'), 'utf8');
  ...
  const ladder = (d.saleDate && d.saleDate !== 'TBD')
    ? [...d.stats, { l: 'Sale Date', v: d.saleDate }]
    : d.stats;
  const stats = ladder.map((s) =>
    `<div class="cl-stat"><div class="cl-stat-l">${esc(s.l)}</div><div class="cl-stat-v">${esc(s.v)}</div></div>`).join('');
  const brokers = d.contacts.map((c) =>
    `<div class="cl-pc"><img src="${asset(c.photo)}" alt=""><div>` +
    `<div class="cl-pc-name">${esc(c.name)}</div><div class="cl-pc-title">${esc(c.title)}</div>` +
    `<div class="cl-pc-meta">${esc(c.phone)} · ${esc(c.email)}</div></div></div>`).join('');

  const html = `<!doctype html><html><head><meta charset="utf-8">
    <style>${base}\n${portfolioCss}</style></head><body>
    <div class="flyer-page cl-page">
      ...
      <div class="cl-panel">
        <div class="cl-address">${esc(d.subtitle)}</div>
        <div class="cl-dash"></div>
        <div class="cl-status">${esc(d.status)}</div>
        <div class="cl-name">${esc(d.name)}</div>
        <div class="cl-stats">${stats}</div>
        <div class="cl-pc-block"><div class="cl-pc-tag">Transaction Brokered By</div>${brokers}</div>
      </div>
    </div></body></html>`;
  ...
  const png = path.join(OUTDIR, `66-norton-closing.png`);
  ...
  const pdfOut = path.join(OUTDIR, `66-norton-closing.pdf`);
