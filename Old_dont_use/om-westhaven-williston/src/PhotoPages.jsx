...
/* Tiles (and hero) accept either a src string or { src, pos } where `pos` is an
   objectPosition override for that one photo (e.g. 'center bottom' to crop sky
   instead of the building). */
export function PhotoGallery({ section, title, accent, subtitle, hero, tiles = [], pageNum }) {
  const Img = ({ photo }) => {
    const { src, pos } = typeof photo === 'string' ? { src: photo } : photo
    return (
