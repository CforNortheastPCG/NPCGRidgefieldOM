...
const SUBJECT = { lat: 42.45835, lng: -71.35350 } // 63 Cottage Lane, Concord MA
const CENTER = { lat: 42.42, lng: -71.46 }            // centered on the 60-min band reach
const ZOOM = 8, W = 640, H = 460

const CITIES = [
  { name: 'Lexington', drive: '~10 mi · 20 min' },
  { name: 'Cambridge', drive: '~17 mi · 30 min' },
  { name: 'Lowell', drive: '~15 mi · 25 min' },
  { name: 'Downtown Boston', drive: '~20 mi · 35 min' },
  { name: 'Logan (BOS)', drive: '~25 mi · 45 min' },
  { name: 'Worcester', drive: '~35 mi · 45 min' },
]
const BAND_HEX = { 15: '#229954', 30: '#2471A3', 45: '#7D3C98', 60: '#C0392B' }

// Bold basemap geometry WITH town + highway labels baked in (no separate overlay
// layer for this deck). The isochrone bands draw on top once regenerated for
// Boston via `npm run isochrones`.
const BASE_STYLE = [
  ...
  'feature:administrative.neighborhood|element:labels|visibility:on',
]
    ...
    <div className="page">
      ...
      <div className="section--tight" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        ...
        <div style={{ fontSize: 11.5, lineHeight: 1.5, color: 'var(--graphite)', marginBottom: 10 }}>
          <strong>Road-network and rail reach from the property.</strong> Route 2 and nearby I-95 / Route 128 connect
          Concord to Cambridge and Boston in roughly 30 to 40 minutes by car, while the MBTA Fitchburg Line offers a
          direct one-seat commuter-rail ride from the Concord depot &mdash; drawing on the deep Greater Boston labor and
          tenant pool.
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '4px 16px', marginBottom: 10, paddingBottom: 8, borderBottom: '1px solid var(--border)' }}>
          <LegendRow color="#F8971D" label="Cottage Lane Multifamily (subject)" pin />
        ...
        <div style={{ position: 'relative', borderRadius: 4, overflow: 'hidden', border: '1px solid var(--border)', flex: 1, minHeight: 0 }}>
          {ok ? (
            <>
              <img src={baseUrl} alt="Drive-time map centered on Cottage Lane Multifamily, Concord MA" style={{ ...fill, objectFit: 'fill', display: 'block' }} />
