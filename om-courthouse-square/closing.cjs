...
// ── deal config ─────────────────────────────────────────────────────────────
const DEAL = {
  slug: 'Courthouse-Square',           // output filename prefix
  status: 'Just Sold',                 // gold ribbon label
  // big-title overlay (all three posts) — info only, no financials
  eyebrow: '22-Unit Multifamily Property',
  title: 'Courthouse<br>Square',
  addr: 'Westfield, MA',
  // photo focus: object-position used by background-position (x% y%)
  focus:         '54% center',         // square (email + linkedin)
  focusPortrait: '54% center',         // instagram portrait
};
