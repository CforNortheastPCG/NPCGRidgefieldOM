...
async function renderPdf({
  ...
  await page.setViewport({ width: 1000, height: 1200, deviceScaleFactor: dsf });
  ...
  console.log(`Found ${pages.length} pages · DSF ${dsf} (~${Math.round(816 * dsf / 8.5)} DPI) · JPEG q${quality} · brighten ${brighten} · cover ${cover}`);
  ...
  // Compose: one portrait 8.5x11 page per screenshot, referenced by file:// so
  ...
  fs.writeFileSync(htmlPath, `
    <html><style>
      * { margin: 0; padding: 0; }
      ...
      .pdf-page { width: 8.5in; height: 11in; page-break-after: always; display: flex; align-items: center; justify-content: center; background: white; }
