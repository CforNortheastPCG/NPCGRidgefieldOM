/* Drive-time band generator for The Granary, New Milford — raw road-following
   polygon method (kept per request). Trace Valhalla's raw isochrone outline
   directly (costing:auto, so it follows the road network), decimate to even
   spacing and Chaikin-smooth for a clean filled band that hugs actual road reach.
   The radial-envelope helpers below (destPoint/bearingFrom/fillGaps/medianFilter/
   movingAvg + BINS/MEDIAN/SMOOTH/CHAIKIN) are retained for the alternate inland
   "organic ring" look; the raw-polygon block at the bottom is what runs here.
   One /isochrone call — fast to re-run.  npm run isochrones */
import { writeFileSync } from 'node:fs'

const SUBJECT = { lat: 41.5743564, lon: -73.4116566 } // 29 West Street, New Milford CT
const CONTOURS = [
...
/* COASTAL / RAW-POLYGON METHOD — trace Valhalla's raw road-following isochrone
   outline directly (decimate to even spacing + Chaikin-smooth). Kept per request:
   this outline hugs actual road reach along the highways rather than smoothing to
   an organic envelope. (See DRIVE_TIMES_MAP_FINAL.md → "raw road-following polygon".) */
...
const file = `/* AUTO-GENERATED by scripts/gen-isochrones.mjs (raw road-following polygon) —
   do not hand-edit. Drive-time bands from 29 West Street, New Milford, CT: raw
