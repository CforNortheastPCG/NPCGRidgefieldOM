import type { HighlightsContent } from './types.ts'

// AGENT-WRITABLE. Set generated: true when authored for this deal.
export const HIGHLIGHTS: HighlightsContent = {
  generated: true,
  groups: [
    {
      title: 'Income & Value-Add Upside',
      items: [
        {
          head: '7.18% In-Place Cap Rate',
          body: 'At the $4,000,000 asking price, in-place net operating income of $287,185 delivers a 7.18% going-in yield, rising to 8.01% on a $320,254 pro forma.',
        },
        {
          head: 'Built-In Rent Upside',
          body: 'Marking units to market lifts gross scheduled rent from $421,200 to $453,600 — a $32,400 annual gain with no change to the physical plant.',
        },
        {
          head: 'Ancillary Income Streams',
          body: 'Coin-op laundry adds $3,302 of underwritten income, and a rooftop leased to three wireless carriers (antennas assessed at $21,000) sits outside the numbers, while tenant-paid electric heat keeps owner costs low.',
        },
      ],
    },
    {
      title: 'Location & Basis',
      items: [
        {
          head: 'Across from Downtown Boston',
          body: 'Chelsea sits opposite downtown Boston across the Mystic River — a routed ~6 miles and about 11 minutes by car, with Revere roughly 1 mile from the door.',
        },
        {
          head: 'Ringed by the Inner Core',
          body: 'Everett, Revere, Somerville, and Boston all border the site; Somerville is a routed ~6 miles and about 13 minutes, keeping the property central to Boston’s inner suburbs.',
        },
        {
          head: '$250,000 Per Unit Basis',
          body: 'The $4,000,000 price pencils to $250,000 per unit and $276.28 per square foot for 14,478 square feet of brick, 1978-vintage apartments.',
        },
      ],
    },
  ],
}
