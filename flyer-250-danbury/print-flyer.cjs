/* Flyer exporter — screenshots each .flyer-page at 2× and composes a
   landscape 11×8.5 PDF (one page per side), plus a PNG per page for the web.
   Usage: node print-flyer.cjs [port] */
const fs = require('fs');
const os = require('os');
...
(async () => {
  ...
  console.log(`Found ${pages.length} flyer pages`);
  const files = [];
  ...
  // Compose a landscape PDF, one flyer page per PDF page.
