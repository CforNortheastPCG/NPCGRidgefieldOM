...
if (require.main === module) {
  // This deck has no raster control to collide with, so pdfName is used as-is
  // (the OM folders append a '-vector' suffix to keep the two side by side).
  const file = path.resolve(process.argv[2] || require('./print-vector.cjs').dealPdfName());
