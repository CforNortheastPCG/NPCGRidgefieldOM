...
async function renderPdf({
  ...
  const browser = await puppeteer.launch({
    ...
    // NOTE: --force-color-profile=srgb was removed — in headless page.pdf() it
    // tints semi-transparent overlays (the cover's black scrim gradient) magenta.
    // Default color handling renders the gradient as true black.
    args: [],
  });
