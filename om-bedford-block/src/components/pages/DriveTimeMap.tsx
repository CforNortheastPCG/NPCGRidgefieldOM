...
export function DriveTimeMap({ pageNum }: { pageNum?: number }) {
  ...
  if (!ready) {
    ...
    // Fan out colliding chips (nested rings can cross the west corridor at
    // nearly the same point) — push later chips down until they clear.
    {
      const placed: Array<{ x: number; y: number }> = []
      for (const c of bandChips) {
        while (placed.some((p) => Math.abs(p.x - c.x) < 62 && Math.abs(p.y - c.y) < 20)) c.y = Math.min(c.y + 22, H - 14)
