  ...
  return (
    <div className="page">
      ...
      <div className="section--tight" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        ...
        {curated && (
          <div style={{ fontSize: 12.1, lineHeight: 1.5, color: 'var(--graphite)', marginBottom: 8 }}>{c.intro}</div>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.15fr 1fr', gap: 16, flex: 1, minHeight: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 7, minHeight: 0, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingLeft: 2, paddingBottom: 7, borderBottom: '1px solid var(--border)' }}>
              <span className="pin-ring" style={{ flexShrink: 0, width: 14, height: 14, borderRadius: '50%', background: '#F8971D', border: '2px solid #fff', boxShadow: '0 0 0 1px var(--golden)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8.5, fontWeight: 800, color: '#fff', lineHeight: 1 }}>
                P
              </span>
            ...
            <div style={{ columns: 2, columnGap: 18, flex: 1, minHeight: 0, overflow: 'hidden' }}>
              {groups.map((g) => (
                <div key={g.heading} style={{ breakInside: 'avoid', marginBottom: 6 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                    <span style={{ flexShrink: 0, width: 8, height: 8, borderRadius: '50%', background: g.swatch, border: '1px solid rgba(0,0,0,0.15)' }} />
                    <span style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', color: g.swatch }}>{g.heading}</span>
                  </div>
                  {g.items.map((p, k) => (
                    <div key={`${g.heading}-${k}`} style={{ display: 'flex', alignItems: 'baseline', gap: 6, padding: '1px 0' }}>
                      <span style={{ flexShrink: 0, fontSize: 12, fontWeight: 700, color: g.swatch, width: 13, textAlign: 'right' }}>{p.n > 0 ? p.n : '·'}</span>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--carbon)', lineHeight: 1.2 }}>{p.name}</div>
                        {p.note && <div style={{ fontSize: 10, color: 'var(--stone)', lineHeight: 1.25, marginTop: 0.5 }}>{p.note}</div>}
                      </div>
                    </div>
                  ))}
                </div>
