...
try {
  ...
  /* Page.printToPDF over CDP with ReturnAsBase64 — page.pdf()'s default
     streamed transfer (IO.read) fails intermittently on this ~28MB book. */
  const cdp = await page.createCDPSession()
  const { data: pdfB64 } = await cdp.send('Page.printToPDF', {
    paperWidth: 960 / 96,
    paperHeight: 742 / 96,
