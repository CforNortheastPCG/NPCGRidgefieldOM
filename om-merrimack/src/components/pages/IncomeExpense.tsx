...
// Dense variant: this deal's workbook carries 18 expense lines + a
// GPR → GSR income bridge across three columns — rows must stay tight.
const tds: CSSProperties = { fontSize: 10, padding: '3.5px 8px', textAlign: 'right' }
const tdl: CSSProperties = { fontSize: 10, padding: '3.5px 8px', textAlign: 'left', fontWeight: 600, color: 'var(--carbon)' }
const thr: CSSProperties = { fontSize: 9, padding: '5px 8px', textAlign: 'right', color: '#fff' }
const thl: CSSProperties = { fontSize: 9, padding: '5px 8px', textAlign: 'left', color: '#fff' }
const totBg: CSSProperties = { background: 'var(--carbon)', color: '#fff', fontWeight: 700, fontSize: 10, padding: '4px 8px', textAlign: 'right' }
const noiBg: CSSProperties = { background: 'var(--golden)', color: '#fff', fontWeight: 800, fontSize: 10.5, padding: '5px 8px', textAlign: 'right' }
...
export function IncomeExpense({ pageNum }: { pageNum?: number }) {
  ...
  const valueCells = (amounts: Record<string, number | undefined>, bold = false, negate = false) =>
    cols.flatMap((c) => {
      ...
      const neg = negate && v !== 0 // never print "-$0"
      const shown = neg ? -v : v
      return [
      ...
      <div className="section--tight" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 8, tableLayout: 'fixed' }}>
          ...
          <tbody>
            ...
            <tr>
              <td style={tdl}>Vacancy &amp; Collections Loss</td>
              {valueCells(byCol((k) => computed.byColumn[k]?.vacancyLoss), false, true)}
            </tr>
            <tr style={{ background: 'var(--linen)' }}>
              <td style={{ ...tdl, fontWeight: 700 }}>Effective Rental Income</td>
              {valueCells(byCol((k) => computed.byColumn[k]?.effectiveRentalIncome), true)}
            </tr>
