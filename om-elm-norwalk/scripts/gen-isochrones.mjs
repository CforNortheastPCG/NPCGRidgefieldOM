/* Drive-time band generator for 6 Elm Street, Norwalk — COASTAL raw-polygon method.

   The property is in South Norwalk on the Long Island Sound shoreline, so a
   point-centered radial ring is the wrong shape (it fills the water wedge between
   the east/west coastal arms). Instead we take Valhalla's raw road-following
   isochrone polygon — which, because costing:auto only follows roads, already
   EXCLUDES the water — then decimate it to even spacing and Chaikin-smooth it for
   a clean filled band.
...
const SUBJECT = { lat: 41.1143093, lon: -73.4167832 } // 6 Elm Street, Norwalk CT (South Norwalk)
const CONTOURS = [
...
const res = await fetch(`${VALHALLA}?json=${encodeURIComponent(JSON.stringify(body))}`, { headers: { 'User-Agent': 'npcg-om-elm-norwalk/1.0' } })
...
/* COASTAL METHOD — raw road-following polygon (not the radial envelope).
   ...
   clean enough to render as a filled band. (See DRIVE_TIMES_MAP_FINAL.md → "Coastal".) */
...
const file = `/* AUTO-GENERATED by scripts/gen-isochrones.mjs (coastal raw-polygon method) —
   do not hand-edit. Drive-time bands from 6 Elm Street, Norwalk (South Norwalk):
   raw Valhalla road-following isochrone polygons (costing auto, so they exclude
   Long Island Sound), decimated + Chaikin-smoothed, as [lat,lng] arrays. Largest
   contour first. Re-run: npm run isochrones */
