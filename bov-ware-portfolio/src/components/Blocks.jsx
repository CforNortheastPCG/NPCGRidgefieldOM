...
export function KpiTile({ label, value, sub, invert = false, style }) {
  return (
...
export function KpiStrip({ items, style }) {
  return (
    <div style={{
...
export function KpiRail({ items, gap = 11, style }) {
  return (
...
export function OwnerBadge({ owner }) {
  return (
    <span style={{
      ...
      color: OWNER_COLORS[owner] || 'var(--stone)', whiteSpace: 'nowrap', flexShrink: 0,
    }}>{owner}</span>
  )
}
