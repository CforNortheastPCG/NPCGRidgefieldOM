...
import { SectionTitle, BlockLabel, KpiStrip, Callout, SourceNote } from '../components/Blocks.jsx'
...
import { ASSESS, TAX, VAL, RR, fmtMoney, fmtMoneyShort, fmtNum, fmtPct } from '../lib/calc.js'
...
export default function AssessmentTaxPage({ pageNum }) {
  ...
  const taxFor = id => (TAXES?.byParcel || []).find(x => x.parcelId === id)?.tax ?? null
    ...
    <div className="page">
      ...
      <div className="section--tight" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          ...
          <tbody>
            {parcels.map((p, i) => (
              <tr key={p.id} style={zebra(i)}>
                ...
                <td style={T.tdr}>{taxFor(p.id) != null ? fmtMoney(taxFor(p.id)) : DASH}</td>
              </tr>
            ))}
            <tr>
              ...
              <td style={T.totr}>{fmtMoney(ASSESS.land)}</td>
              <td style={T.totr}>{fmtMoney(ASSESS.buildings)}</td>
              <td style={T.totr}>{fmtMoney(ASSESS.total)}</td>
                      ...
                      return (
                        <tr key={r.label}>
                          <td style={{ ...T.tdl, ...z, fontWeight: base ? 800 : 600 }}>
                            {base ? 'As billed today' : `At ${Math.round(r.ratio * 100)}% of price`}
                          </td>
        ...
        <SourceNote>
          ...
          {' '}Assessed value equates to {fmtMoney(ASSESS.perUnit)} per unit across {RR.unitCount} units.
        </SourceNote>
      </div>
