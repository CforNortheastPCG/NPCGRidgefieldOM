...
import { SectionTitle, BlockLabel, Callout, SourceNote } from '../components/Blocks.jsx'
import { T, zebra } from '../components/tableKit.js'
import { LadderCurve, RangeBar } from '../components/Charts.jsx'
import { VALUATION } from '../data/valuation.js'
import { FIN, VAL, RR, fmtMoney, fmtMoneyShort, fmtPct, fmtPctFixed } from '../lib/calc.js'
...
export default function ValuationPage({ pageNum }) {
  ...
  const debtRows = debt ? [
    ...
    ...(FIN.hasT12 ? [['Net Operating Income — Trailing 12', fmtMoney(FIN.t12.noi)]] : []),
  ...
  const cards = [
    { title: 'Pricing Rationale', body: VALUATION.pricingRationale },
    { title: 'Likely Buyer Profile', body: VALUATION.buyerProfile },
    { title: 'What Moves the Price', body: VALUATION.whatMovesPrice },
  ].filter(c => c.body)
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, marginBottom: 12, flex: 1, minHeight: 0 }}>
          ...
          <div style={{ minWidth: 0, display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ marginTop: 9 }}>
              ...
              <RangeBar
                low={VAL.ladder[VAL.ladder.length - 1].price}
                high={VAL.ladder[0].price}
                mark={VAL.ask}
                fmt={fmtMoneyShort}
                lowLabel={`at ${fmtPctFixed(VAL.ladder[VAL.ladder.length - 1].cap)}`}
                highLabel={`at ${fmtPctFixed(VAL.ladder[0].cap)}`}
                markLabel="ASK"
                height={50}
              />
            </div>
