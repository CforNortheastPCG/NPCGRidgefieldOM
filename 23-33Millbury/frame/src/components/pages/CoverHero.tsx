  ...
  return (
    <div className="page">
      ...
      <div className="cover-hero">
        ...
        {/* The title stack — movable via cover tweaks, but always this
            order (om-coppshill canon): status / name / address / town /
            rule / two tag lines (asset descriptor + authored subtitle). */}
        <div className="cover-hero-overlay" style={{ ...cornerBox(titleCorner), position: 'absolute' }}>
          ...
          <div className="cover-hero-name" style={{ fontSize: 42, whiteSpace: 'nowrap' }}>{DEAL.name}</div>
          {DEAL.address.trim().toLowerCase() !== DEAL.name.trim().toLowerCase() && (
            <div className="cover-hero-title">{DEAL.address}</div>
          )}
          <div className="cover-hero-sub">{DEAL.cityLong}</div>
          <div className="cover-hero-rule" style={alignRight ? { marginLeft: 'auto' } : undefined} />
          <div className="cover-hero-prep">
            {[DEAL.descriptor, COVER.subtitle].filter(Boolean).join(' · ')}
          </div>
        </div>
      </div>
