...
// Cover tag line #1 — deterministic asset descriptor in the shipped-book
// style ("16-Unit Multifamily · ±14,478 SF"). Built from whatever the
// payload actually has; never invents.
const descriptorParts = []
descriptorParts.push(
  [property.unitCount ? `${property.unitCount}-Unit` : null, PROPERTY_TYPE_LABELS[deal.propertyType] ?? deal.propertyType]
    .filter(Boolean)
    .join(' ')
)
if (property.buildingSf) descriptorParts.push(`±${int(property.buildingSf)} SF`)
...
// --keep-maps: warm-sandbox re-injects (photo swaps) must not blank the
// generated maps — there is no snapshot-restore after them. A fresh/full
// inject still resets to the placeholder (gen-maps or restore refills it).
if (!(args.includes('--keep-maps') && existsSync(join(dataDir, 'mapdata.ts')))) {
  emit(
    'mapdata.ts',
...
// --keep-tweaks: same warm-sandbox rule for broker layout tweaks.
if (!(args.includes('--keep-tweaks') && existsSync(join(dataDir, 'tweaks.ts')))) {
  emit(
    'tweaks.ts',
