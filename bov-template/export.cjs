...
/* ═══════════════════ ONE-COMMAND BOV EXPORT (RASTER) ═══════════════════
   `npm run pdf` → builds the site, serves the production bundle, renders the
   image-only PDF (print.cjs), and tears the server down. Self-contained: no
   dev server needs to be running. All print.cjs env knobs apply, e.g.:
     QUALITY=55 npm run pdf      (smaller file)
     COVER=1.28 npm run pdf      (lift a still-dark printed cover) */

// Every deck folder is a clone and ALL default to 4173 — so if a sibling
// deck's preview/dev server is already up, we'd silently screenshot the WRONG
// deck. Pick a genuinely-free port (probe upward from the requested one) and
// verify the served deck is ours before rendering. PORT env still forces a
// specific port if you need one.
...
// Invoke vite via `node <vite.js>` rather than the .bin shim (shell-free,
