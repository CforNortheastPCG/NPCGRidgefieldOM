...
try {
  ...
  /* Artifact guard — CSS effects with no PDF primitive (text-shadow,
     filter, blend modes…) get rasterized by Skia into soft-mask groups
     that Preview/Quick Look and some in-app viewers composite as solid
     black or pink boxes. Computed style under print emulation is exactly
     what Skia will see. Advisory: warn loudly, print anyway.
     Full playbook: docs/PDF-ARTIFACTS.md */
  const unsafe = await page.evaluate(() => {
    ...
    for (const el of document.querySelectorAll('.page, .page *')) {
      ...
      seen.add(sig)
      out.push(`${el.closest('[id^="p-"]')?.id ?? '?'}: ${sig}`)
    }
    return out.slice(0, 20)
  })
  if (unsafe.length > 0) {
    console.error(`! print-unsafe CSS on ${unsafe.length} element type(s) — Skia rasterizes these into soft-mask groups; some PDF viewers show pink/black boxes. Fix: disable under @media print (docs/PDF-ARTIFACTS.md).`)
  ...
  try {
    ...
    /* COLORSPACE NORMALIZE (docs/PDF-ARTIFACTS.md): Skia tags every rendered
       image with a compact ICC v4 profile via [/ICCBased n 0 R]. Chromium
       reads it, but Acrobat / macOS Preview / print RIPs can fail the v4
       parse and render photos PINK. The pixels are plain sRGB, so swap each
       ICCBased colorspace to the matching /Device* (3ch→RGB, 1ch→Gray,
       4ch→CMYK) — universal, visually identical. */
    {
