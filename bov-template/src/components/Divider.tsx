import { FULL_ADDR, assetUrl } from './Shell.tsx'
  ...
  return (
    <div className="page">
      <div className="cover-hero" style={{ background: 'var(--carbon)' }}>
        ...
        <img className="cover-hero-img" src={assetUrl(image)} alt="" style={{ opacity: 0.3 }} />
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(24,28,34,0.45)' }} />
        <div className="cover-hero-header">
          <img src={assetUrl('/logos/npcg-white-hires.png')} alt="NPCG" style={{ maxHeight: 40, maxWidth: 200, objectFit: 'contain' }} />
        </div>
        ...
        <div style={{
          ...
          {eyebrow && (
            <div style={{ marginBottom: 11, color: 'var(--golden)', fontSize: 12, fontWeight: 700, letterSpacing: '0.28em' }}>
              {eyebrow}
            </div>
          )}
          <div className="cover-hero-title" style={{ fontSize: 48 }}>{title}</div>
          <div className="cover-hero-rule" style={{ margin: '16px auto 0' }} />
          <div className="cover-hero-prep" style={{ marginTop: 14 }}>{FULL_ADDR}</div>
        </div>
        {pageNum != null && (
          <div style={{ position: 'absolute', right: 26, bottom: 18, color: '#fff', fontSize: 9.3, fontWeight: 700, opacity: 0.85, letterSpacing: '0.04em' }}>
            {pageNum}
          </div>
        )}
      </div>
