import type { LabelValue, PropertyData } from '../../lib/types.ts'
...
function Card({ title, rows = [], flex }: { title: string; rows?: LabelValue[]; flex?: number }) {
  return (
    <div className="bldg-card" style={{ padding: '7px 12px', flex, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
      <h3 style={{ fontSize: 9.3, marginBottom: 4, paddingBottom: 3 }}>{title}</h3>
...
export default function PropertyPage({ property, showName = false, pageNum }: {
  ...
  /** Portfolio decks print the property name above the cards. */
  showName?: boolean
  pageNum?: number
}) {
  const p = property
