...
import { FaqBlock } from '../Blocks.tsx'
import { EXPECT } from '../../data/process.ts'
...
export default function ExpectPage({ pageNum }: { pageNum?: number }) {
  return (
    <div className="page">
      ...
      <div className="section--tight" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        ...
        <div style={{ fontSize: 9.5, lineHeight: 1.5, color: 'var(--graphite)', marginBottom: 8 }}>{EXPECT.intro}</div>

        <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 19, flex: 1, minHeight: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: 0 }}>
            {EXPECT.milestones.map((m, i) => (
              <div key={m.title} style={{ display: 'flex', gap: 9, alignItems: 'flex-start' }}>
                ...
                <div style={{ paddingBottom: 7, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                    <span style={{ fontSize: 9.5, fontWeight: 800, color: 'var(--carbon)' }}>{m.title}</span>
                    <span style={{ fontSize: 7, fontWeight: 800, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--terracotta)', whiteSpace: 'nowrap' }}>{m.decision}</span>
                  </div>
