...
export default function MarketActivity({ pageNum }: { pageNum?: number }) {
  ...
  return (
    <div className="page">
      ...
      <div className="section--tight" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        ...
        <div style={{ fontSize: 9.2, lineHeight: 1.5, color: 'var(--graphite)', marginBottom: 9 }}>{a.intro}</div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, flex: 1, minHeight: 0 }}>
          {/* New development */}
          <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            <h3 style={{ fontSize: 9.2, fontWeight: 700, color: 'var(--carbon)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8, paddingBottom: 5, borderBottom: '2px solid var(--golden)' }}>New &amp; Notable Development</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flex: 1, justifyContent: 'flex-start' }}>
              {a.developments.map(d => (
                <div key={d.title} style={{ borderLeft: '3px solid var(--golden)', paddingLeft: 11 }}>
                  <div style={{ fontSize: 9.7, fontWeight: 800, color: 'var(--carbon)' }}>{d.title}</div>
          ...
          {/* Laws & regulations */}
          <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            <h3 style={{ fontSize: 9.2, fontWeight: 700, color: 'var(--carbon)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8, paddingBottom: 5, borderBottom: '2px solid var(--golden)' }}>Laws &amp; Regulations to Know</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flex: 1, justifyContent: 'flex-start' }}>
              {a.regulations.map(r => (
                <div key={r.title} style={{ borderLeft: '3px solid var(--carbon)', paddingLeft: 11 }}>
                  <div style={{ fontSize: 9.7, fontWeight: 800, color: 'var(--carbon)' }}>{r.title}</div>
