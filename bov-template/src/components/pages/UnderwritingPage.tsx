...
import { SectionTitle, BlockLabel, KpiRail, SourceNote } from '../Blocks.tsx'
import { Waterfall, DonutChart } from '../Charts.tsx'
import { T, zebra, DASH } from '../tableKit.ts'
import { FIN_NOTES, T12_LABEL, VACANCY_PCT } from '../../data/financials.ts'
import { FIN, RR, fmtMoney, fmtMoneyShort, fmtPct } from '../../lib/calc.ts'
...
/* Declared at module scope, not inside the page component: a component
   ...
   closed over. */
/* The three columns. `Column` is the union the row accessors see: a
   trailing-twelve column carries nulls where an underwriting carries
   numbers, and the row renders an em dash rather than an invented zero. */
type MarketColumn = typeof FIN.current
type GivenColumn = NonNullable<typeof FIN.t12>
type Column = MarketColumn | GivenColumn

type Money = number | null | undefined

const m = (n: Money) => (n == null ? DASH : fmtMoney(n))
// Parenthesised negatives — accounting convention, and it stops a reader
// mistaking a deduction for income at a glance.
const neg = (n: Money) => (n == null ? DASH : `(${fmtMoney(Math.abs(n))})`)

const Cols = ({ hasT12 }: { hasT12: boolean }) => {
...
const Head = ({ hasT12 }: { hasT12: boolean }) => (
  <thead>
    <tr style={T.head}>
      <th style={T.thl} />
      {hasT12 && <><th style={T.thr}>As Given</th><th style={T.thr}>{PER.label}</th></>}
      <th style={T.thr}>Market</th><th style={T.thr}>{PER.label}</th>
      <th style={T.thr}>Pro Forma</th><th style={T.thr}>{PER.label}</th>
    </tr>
  </thead>
)
...
/* The per-space column. Classes that trade per unit (multifamily, SRO,
   small-bay industrial) divide by the space count; retail and office divide
   by area, because that is the figure their market quotes. */
const PER = VOCAB.perUnitPrice
  ? { label: `/ ${VOCAB.unit}`, of: (v: number) => FIN.perUnit(v) }
  : { label: '/ SF', of: (v: number) => FIN.perSf(v) }

const Row = ({ label, get, fmt = m, style, i = 0, noPerUnit = false, hasT12 }: {
  label: string
  get: (s: Column) => Money
  fmt?: (n: Money) => string
  style?: { l: CSSProperties; r: CSSProperties }
  i?: number
  /** Blank the /unit cells where dividing by the unit count is meaningless
      — a ratio per unit is not a number. */
  noPerUnit?: boolean
  hasT12: boolean
}) => {
  const cell = (sc: Column | null, isPu = false) => {
    if (!sc) return DASH
    const v = get(sc)
    // Use the SAME formatter for the /unit cell, so a parenthesised
    // negative stays parenthesised rather than turning into "$-2,200".
    if (isPu) return noPerUnit || v == null ? DASH : fmt(PER.of(v))
    return fmt(v)
  }
...
const SLICE_COLORS = ['var(--carbon)', 'var(--golden)', 'var(--terracotta)', 'var(--stone)', '#8d95a1', '#d8d2cb'] as const

export default function UnderwritingPage({ pageNum }: { pageNum?: number }) {
  ...
  const expenseSlices = (() => {
    ...
    return out.map((e, i) => ({ ...e, color: SLICE_COLORS[i % SLICE_COLORS.length] ?? SLICE_COLORS[0] }))
  })()
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 218px', gap: 16, flex: 1, minHeight: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0, gap: 12 }}>
            <div>
              ...
              <Waterfall
                steps={[
                  ...(FIN.t12 ? [{ label: 'As Given', value: FIN.t12.noi, type: 'base' }] : []),
