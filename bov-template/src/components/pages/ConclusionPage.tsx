import { PageHeader, PageFooter, Md, DEAL, PlaceholderBanner } from '../Shell.tsx'
import { SectionTitle, BlockLabel } from '../Blocks.tsx'
import { zebra } from '../tableKit.ts'
import { VALUATION } from '../../data/valuation.ts'
import { CONCLUSION_CONTENT } from '../../content/index.ts'
import { ADVISORS } from '../../data/advisors.ts'
import { OFFICES } from '../../data/firm.ts'
import { VAL, FIN, fmtMoney, fmtMoneyShort, fmtPct } from '../../lib/calc.ts'
import { VOCAB } from '../../lib/vocab.ts'
import { RangeBar } from '../Charts.tsx'
...
export default function ConclusionPage({ pageNum }: { pageNum?: number }) {
  const caps = [...VALUATION.capScenarios].sort((a, b) => a.cap - b.cap)
  // Highest cap → lowest value, and vice versa. With no scenarios supplied
  // both ends collapse to the ask's own implied cap rather than dividing by
  // an undefined one.
  const askCap = { label: 'Market', cap: VAL.askMetrics.capCurrent }
  const lowCap = caps[caps.length - 1] ?? askCap
  const highCap = caps[0] ?? askCap
  ...
  return (
    <div className="page">
      ...
      {!CONCLUSION_CONTENT.generated && <PlaceholderBanner what="the stated opinion" />}
      <div className="section--tight" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 258px', gap: 20, flex: 1, minHeight: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
            ...
            {(CONCLUSION_CONTENT.opinionParagraphs || []).slice(0, 1).map((p, i) => (
            ...
            <div style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 'var(--fs-note)', color: 'var(--stone)', marginTop: 5 }}>
                  {VOCAB.perUnitPrice && <>{fmtMoney(VAL.askMetrics.perUnit)} per {VOCAB.unit.toLowerCase()} &nbsp;|&nbsp; </>}${VAL.askMetrics.perSf.toFixed(2)} per SF
            ...
            {(CONCLUSION_CONTENT.opinionParagraphs || []).slice(1).map((p, i) => (
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
              ...
              <div style={{ fontSize: 'var(--fs-note)', lineHeight: 1.4, color: 'var(--stone)', paddingTop: 10 }}>
                {CONCLUSION_CONTENT.disclaimerNote}
              </div>
            </div>
          </div>
