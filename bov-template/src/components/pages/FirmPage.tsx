...
import { PageHeader, PageFooter, Md, assetUrl } from '../Shell.tsx'
import { SectionTitle, BlockLabel, KpiStrip, SourceNote } from '../Blocks.tsx'
import {
  OFFICES, LEADERSHIP, SENIOR_INVESTMENT_SALES, INVESTMENT_SALES,
  SUPPORT_STAFF, FIRM_STATS, FIRM_NARRATIVE, CORE_VALUES,
} from '../../data/firm.ts'
import { TRACK_RECORD } from '../../data/trackRecord.ts'
...
function Face({ member, size = 54 }: { member: TeamMember; size?: number }) {
  const common: CSSProperties = {
    width: size, height: size, borderRadius: '50%', objectFit: 'cover',
    display: 'block', margin: '0 auto 6px', border: '2px solid var(--golden)',
  }
  if (member.photo) return <img src={assetUrl(member.photo)} alt={member.name} style={common} />
  return (
    <div style={{
...
const Row = ({ people, size }: { people: TeamMember[]; size?: number }) => (
...
export default function FirmPage({ pageNum }: { pageNum?: number }) {
