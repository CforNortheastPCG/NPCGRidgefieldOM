...
/** An office pin, in real geography — projected, never eyeballed. */
...
export default function EastCoastMap({ markers = [] }: { markers?: MapMarker[] }) {
  const { dots, projectedMarkers } = useMemo(() => {
    // us-atlas ships plain JSON; topojson's own types describe its shape.
    const topo = statesTopo as unknown as Topology<{ states: GeometryCollection }>
    const all = feature(topo, topo.objects.states) as FeatureCollection<Geometry>
    const eastCoast: FeatureCollection<Geometry> = {
      type: 'FeatureCollection',
      features: all.features.filter((f: Feature<Geometry>) =>
        EAST_COAST_FIPS.has(String(f.id).padStart(2, '0'))
      ),
    }
