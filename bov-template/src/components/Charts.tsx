...
/* ── Chart data contracts. Every chart takes plain rows; the pages compute
      them from src/lib/calc.ts and never hand-total. ── */
...
/** Value formatter — charts render whatever it returns. */
export type Fmt = (v: number) => ReactNode
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1, minWidth: 0 }}>
          {data.map(d => (
            <div key={d.label} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 'var(--fs-note)' }}>
              ...
              <span style={{ flex: 1, color: CARBON, fontWeight: 600, minWidth: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{d.label}</span>
...
export function LadderCurve({ rows, fmt = v => v, height = 104 }: {
  ...
  const x = (c: number) => ((c - minC) / (maxC - minC || 1)) * 100
  const y = (p: number) => height - ((p - minP) / (maxP - minP || 1)) * (height - 14) - 7
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 3, fontSize: 6.8, fontWeight: 700, color: STONE, letterSpacing: '0.06em' }}>
        <span>{(caps[0] ?? 0).toFixed(2)}% · {fmt(prices[0] ?? 0)}</span>
        {ask && <span style={{ color: GOLDEN, fontWeight: 800 }}>ASK {ask.cap.toFixed(2)}% · {fmt(ask.price)}</span>}
        <span>{(caps[caps.length - 1] ?? 0).toFixed(2)}% · {fmt(prices[prices.length - 1] ?? 0)}</span>
      </div>
    </div>
  )
}
