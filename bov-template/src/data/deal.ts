import type { AssetClass, DeckFormat } from '../lib/types.ts'
/* ═══════════════════ EDIT-ME · DEAL CONFIG ═══════════════════
...
/* ── ASSET CLASS ────────────────────────────────────────────────────────
   What kind of property this is. It decides the deck's VOCABULARY (a suite
   is never called a unit) and WHICH RENT-ROLL PAGES render:

     'multifamily'  residential unit roll · Unit Mix & Income
     'sro'          same, but rooms — a room is never called a unit
     'retail'       lease-by-lease roll · Tenant Rent Roll, Lease Rollover,
     'office'       Tenant Profiles. Price leads on $/SF; per-suite price
                    is dropped as meaningless.
     'industrial'   commercial roll, but small-bay industrial does trade per
                    unit, so that metric stays.
     'mixed-use'    BOTH rolls — residential units and commercial suites,
                    each on its own page.

   Commercial classes read src/data/commercialRoll.ts; residential classes
   read src/data/rentRoll.ts. Set the one that matches; the manifest and the
   vocabulary follow. See src/lib/vocab.ts for the table. */
export const ASSET_CLASS: AssetClass = 'multifamily'   // SAMPLE — replace

export const FORMAT: DeckFormat = 'short'                // 'short' | 'full'
