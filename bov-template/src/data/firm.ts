import type { Office, TeamMember } from '../lib/types.ts'
/* ═══════════════════ FIRM DATA ═══════════════════
...
export const SENIOR_INVESTMENT_SALES: TeamMember[] = [
...
export const INVESTMENT_SALES: TeamMember[] = [
...
export const SUPPORT_STAFF: TeamMember[] = [
...
/* ═══════════════════ WHY NPCG (BOV firm page) ═══════════════════
   Stats trace to the firm's Salesforce closed-deal pipeline ("Closed Deal
   Pipeline 2014 - Present") — refresh the pull when they drift. The third
   tile is derived per-deal from trackRecord.js by the WhyNpcg page. */
...
export const CORE_VALUES = [
  { title: 'Relationships First', body: 'Our success is measured in decades of repeat clients — not single transactions. Every engagement is run like it’s the start of a 20-year relationship.' },
  { title: 'Truth Builds Trust', body: 'We tell owners what the market will actually pay, not what they want to hear. Honest pricing is the shortest path to a close.' },
  { title: 'Execution Discipline', body: 'We run a structured call-for-offers process on every assignment. No fishing expeditions, no passive listings — disciplined execution drives price.' },
  { title: 'Knowledge Is Empowering', body: 'Our advisors are territory specialists. We know every owner, every tenant, and every comp in the markets we cover.' },
]

export const FIRM_NARRATIVE = {
  lead: [
    'Ours is a culture that champions teamwork and celebrates each other. Our collaborative approach allows us to put your best interests first.',
  ],
  pullQuote: 'But we don’t want to just sell your building.',
  close: [
    'When you engage Northeast Private Client Group, you are not hiring a single broker — you are engaging the entire firm: an investment sales team working across seven offices from New England to Florida, an investor database built over 750+ closed transactions, and in-house marketing, underwriting, and transaction coordination that carry a deal from valuation through closing.',
