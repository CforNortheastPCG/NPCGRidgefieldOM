  ...
  return (
    <div className="page">
      ...
      <div className="section--tight" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20, margin: '4px 0 14px' }}>
          {stats.map(s => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 26, flex: 1, minHeight: 0 }}>
          {/* Narrative + pull quote */}
          <div style={{ fontSize: 10.3, lineHeight: 1.55, color: 'var(--carbon)' }}>
            {FIRM_NARRATIVE.lead.map((p, i) => <p key={i} style={{ marginBottom: 10 }}>{p}</p>)}
            <p style={{ margin: '12px 0', paddingLeft: 14, borderLeft: '3px solid var(--golden)', fontSize: 13.5, fontWeight: 700, lineHeight: 1.4 }}>
              {FIRM_NARRATIVE.pullQuote}
            </p>
            {FIRM_NARRATIVE.close.map((p, i) => <p key={i} style={{ marginBottom: 10 }}>{p}</p>)}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 10, alignContent: 'start' }}>
            ...
            {CORE_VALUES.map(v => (
              <div key={v.title} style={{ borderLeft: '3px solid var(--golden)', paddingLeft: 11 }}>
                <div style={{ fontSize: 10.5, fontWeight: 800, color: 'var(--carbon)' }}>{v.title}</div>
