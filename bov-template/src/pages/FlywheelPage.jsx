...
import { FLYWHEEL } from '../data/advisory.js'
...
export default function FlywheelPage({ pageNum }) {
        ...
        <div style={{ display: 'grid', gridTemplateColumns: `${SIZE}px 1fr`, gap: 30, flex: 1, minHeight: 0, marginTop: 4 }}>
          ...
          <div style={{ position: 'relative', width: SIZE, height: SIZE, flex: '0 0 auto', alignSelf: 'center' }}>
            <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`} style={{ position: 'absolute', inset: 0, display: 'block' }} aria-hidden="true">
              {stages.map((s, i) => (
                <path
                  key={s.title}
                  d={sectorPath(i)}
                  style={{ fill: FILLS[i % FILLS.length].fill, fillOpacity: FILLS[i % FILLS.length].opacity, stroke: 'var(--white)', strokeWidth: 3 }}
                />
              ))}
            ...
            {stages.map((s, i) => {
              ...
              return (
                <div key={s.title} style={{ position: 'absolute', left: x - 60, top: y - 26, width: 120, textAlign: 'center', color: FILLS[i % FILLS.length].ink }}>
