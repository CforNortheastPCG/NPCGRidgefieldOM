...
export default function PortfolioSummary({ pageNum }) {
  ...
  return (
    <div className="page">
      ...
      <div className="section--tight" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 10, flex: 1, minHeight: 0, marginTop: 12 }}>
          {PROPERTIES.map(p => (
            <div key={p.id} style={{ flex: 1, position: 'relative', borderRadius: 3, overflow: 'hidden', minHeight: 0 }}>
              ...
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(transparent, rgba(40,27,18,0.75))', color: '#fff', padding: '18px 12px 8px', fontSize: 9.5, fontWeight: 700, letterSpacing: '0.06em' }}>{p.name}</div>
            </div>
          ))}
        </div>
      </div>
