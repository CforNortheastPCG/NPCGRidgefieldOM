...
import { T, zebra, DASH, DENSE } from '../components/tableKit.js'
...
export default function AsGivenPage({ pageNum }) {
  if (!AS_GIVEN) return null
  const inc = AS_GIVEN.income || []
          ...
          {[0, 1].map(col => (
            <table key={col} style={{ ...T.table, alignSelf: 'start' }}>
              ...
              <tbody>
                ...
                {col === 1 && (
                  <tr>
                    <td style={T.totl} colSpan={2}>{AS_GIVEN.rentRollTotalLabel || 'Total — Monthly'}</td>
                    <td style={T.totr}>{fmtMoney(AS_GIVEN.rentRollTotal)}</td>
                    <td style={T.totr} colSpan={2}>{fmtMoney(AS_GIVEN.rentRollTotal * 12)} / yr</td>
                  </tr>
                )}
              </tbody>
            </table>
            ...
            <table style={T.table}>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td style={T.totr}>{fmtMoney(AS_GIVEN.statedNoi)}</td>
                </tr>
              </tbody>
            </table>
          </div>
